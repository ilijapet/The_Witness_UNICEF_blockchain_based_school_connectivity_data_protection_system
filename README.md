...
4. Cratesi dapp backend receive encrypted data; decrypts data and confirm that data are coming for valid school device (via basic asymmetric key cryptography) and decode data back into plain text (Python dict)
...
8. From the user perspective (or admin perspective), via React app he can query Cartesi backend for data related to specific device
...
4. Then we will build mock school devices to be used to generate data (acting like schools are submitting) and to feed that data into Cartesi dap backend via the Django REST backend and set of Cratesi contracts.
...
2. Django backend > Cartesi backend relation: Django is used here just to submit and pay for transaction coming from mocked device (this can well done as simple Python script talking directly to Cartesi smart contracts - acting as client)
...
Generate public-private key pairs for school device and Catesi dapp backend. S
...
![Architectural overview](./cartesi_frontend//public/layers.png) Tech stack for this phase of project will be: React frontend, Django REST web2 backend, web3.py, Cartesi tech stack, Github. Git and supporting Python cryptographic and other libraries (pycryptodome, crypto or similar).
...
Video can be checked here ![The Witness: UNICEF data protection network]()
